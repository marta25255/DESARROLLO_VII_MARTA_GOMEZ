...
function clasificarEstudiantes($promedios) {
    $listaTutoria = [];
    $listaHonor = [];

    // Recorremos el arreglo de estudiantes
    foreach ($promedios as $estudiante => $promedio) {
        // Estudiantes que necesitan tutoría (promedio menor a 75)
        if ($promedio < 75) {
            $listaTutoria[] = $estudiante;
        } 
        // Estudiantes de honor (promedio de 90 o más)
        elseif ($promedio >= 90) {
            $listaHonor[] = $estudiante;
        }
    }

    // Mostramos los resultados
    echo "Estudiantes que necesitan tutoría: " . implode(", ", $listaTutoria) . "\n";
    echo "Estudiantes de honor: " . implode(", ", $listaHonor) . "\n";
}
